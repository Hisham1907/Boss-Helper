...
function confirmRemoveImg() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      removeImg();
      if (formMode === "update") {
        const employee = currentUser.employees.find(
...
function deleteEmployee(id) {
  ...
  deleteModalBtn.onclick = function () {
    const employeeIndex = currentUser.employees.findIndex(
      (emp) => emp.id === id
    );
    if (employeeIndex !== -1) {
      ...
      const paginatedEmployees = currentUser.employees.slice(
        startIndex,
...
function searchEmployees() {
  const filteredEmployees = currentUser.employees.filter(
    (employee) =>
      employee.firstName
        .toLowerCase()
...
function displayData(employeesArr) {
  let content = "";
  const startIndex = (currentPage - 1) * entriesPerPage;
  const paginatedEmployees = employeesArr.slice(
    startIndex,
    startIndex + entriesPerPage
  );
                ...
                <tr>
                    ...
                    <td>
                        <div class="table-btns">
                            <i class="fa-solid fa-pen-to-square update" onclick="getData(${
                              employee.id
                            })"></i>
                            <i class="fa-solid fa-trash delete" onclick="deleteEmployee(${
                              employee.id
                            })"></i>
                        </div>
                    </td>
...
function updateFooterText(totalEntries) {
  ...
  const endIndex =
    totalEntries === 0
      ? 0
      : Math.min(startIndex + entriesPerPage - 1, totalEntries);
...
function updatePaginationButtons(filteredEmployees = null) {
  const totalPages = Math.ceil(
    (filteredEmployees || currentUser.employees).length / entriesPerPage
  );
...
function validateFirstName() {
  ...
  if (!nameRegex.test(firstNameValue))
    return setError(firstName, "First Name can only contain letters");
  if (firstNameValue.length < 3)
    return setError(firstName, "Name must be at least 3 characters long");
  if (firstNameValue.length >= 30)
    return setError(firstName, "Name can't be more than 30 characters long");
...
function validateLastName() {
  ...
  if (!nameRegex.test(lastNameValue))
    return setError(lastName, "Last Name can only contain letters");
  if (lastNameValue.length < 3)
    return setError(lastName, "Name must be at least 3 characters long");
  if (lastNameValue.length >= 30)
    return setError(lastName, "Name can't be more than 30 characters long");
...
function validateAge() {
  ...
  if (isNaN(ageValue) || Number(ageValue) <= 0 || Number(ageValue) > 150)
    return setError(age, "Please enter a valid age");
...
function validatePosition() {
  ...
  if (!positionRegex.test(positionValue))
    return setError(position, "Position can only contain letters and spaces");
...
function validateSalary() {
  ...
  if (isNaN(salaryValue) || Number(salaryValue) <= 0)
    return setError(salary, "Please enter a valid salary");
...
function validatePhone() {
  ...
  if (!/^(\+\d{1,3})?(\d{9,15})$/.test(phoneValue))
...
function addTooltips() {
  tippy("#table-size", {
    content: "Select number of entries to show",
    placement: "top",
    zIndex: 9999999,
    theme: "modes",
  });
  tippy("#search", {
    content: "Search for an employee",
    placement: "top",
    zIndex: 9999999,
    theme: "modes",
  });
  tippy(".fa-magnifying-glass", {
